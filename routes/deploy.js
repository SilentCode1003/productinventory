...
const { Deploy, Return, DeployProduct, Product, Employee } = require('./model/spimodel')
...
router.post('/save', (req, res) => {
  try {
    const { assetcontrol, serial, date, deployby, deployto, referenceno } = req.body
    let deploy = [[assetcontrol, serial, date, deployby, deployto, referenceno]]
    console.log(deploy)
        ...
        .then((result) => {
          ...
          Check_Employee(item.deployby)
            .then((result) => {
              ...
              Check_Deploy(product[0].assetcontrol, convertExcelDate(item.date), item.deployto)
                .then((result) => {
            ...
            .catch((error) => {
              console.error('Error: ', error, 'Name:', item.deployby, item.serial)
              return res.json({
                msg: error,
              })
            })
...
function Check_Deploy(assetcontrol, date, deployto) {
  return new Promise((resolve, reject) => {
    let sql = 'select * from deploy where d_assetcontrol=? and d_date=? and d_deployto=?'
