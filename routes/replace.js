...
router.post('/save', (req, res) => {
  try {
    const { assetcontrol, itemserial, replacedserial, remarks, date, replacedby, referenceno } =
      req.body
    ...
    Check_ReplaceItem(assetcontrol, date)
      .then((result) => {
        ...
        if (data.length != 0) {
          ...
          let replaceitem = [
            [assetcontrol, itemserial, replacedserial, remarks, date, replacedby, referenceno],
          ]
          ...
          let sql = 'update product set p_status="REPLACED" where p_assetcontrol=?'
...
function Record_Replaced_Defective(
  redAssetControl,
  redSerial,
  redDate,
  redReferenceNo,
    ...
    Select(command, (err, result) => {
      ...
      let newHistory = {
        REPLACEMENT: {
          date: date,
          details:
            'Replaced: SERIAL_' + replacedAssetcontrol + ', Replacement: SERIAL_' + assetcontrol,
        },
      }

      details.push(newHistory)

      let update_history = 'update sales_report_history set srh_date=? where srh_referenceno=?'
