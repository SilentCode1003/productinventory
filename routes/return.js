...
router.post('/save', (req, res) => {
  try {
    const { assetcontrol, serial, date, returnby, returnfrom, referenceno } = req.body
    let returnitem = [[assetcontrol, serial, date, returnby, returnfrom, referenceno]]
      ...
      Check_Product(item.serial)
        .then((result) => {
          ...
          if (data.length != 0) {
            ...
            Check_Return(assetcontrol, convertExcelDate(item.date)).then((result) => {
              let data = Return(result)
              counter += 1
                ...
                Check_SalesReport(assetcontrol)
                  .then((salesResult) => {
                    ...
                    if (salesResult != 0) {
                      ...
                      let status = GetValue(RET())
                      let report = 'update sales_report set sr_status=? where sr_assetcontrol=?'
