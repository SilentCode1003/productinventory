...
const { Sold, SoldProduct, MasterClient, Product } = require('./model/spimodel')
const { InsertTable, Select, Update, SelectParameter } = require('./repository/spidb')
...
router.post('/save', (req, res) => {
  try {
    ...
    Check_Product(serial).then((result) => {
            ...
            Check_Sold(assetcontrol, convertExcelDate(item.date), soldto)
              .then((result) => {
                ...
                if (check_sold.length != 0) {
                  ...
                  let product_update = 'update product set p_status=? where p_assetcontrol=?'
...
function Check_Sold(assetcontrol, date, soldto) {
  return new Promise((resolve, reject) => {
    let sql = 'select * from sold where s_assetcontrol=? and s_date=? and s_soldto=?'
  ...
  return new Promise((resolve, reject) => {
    ...
    Select(command, (err, result) => {
      ...
      if (data.length != 0) {
        ...
        let master_client = [[_branch, _company, status, createdby, createddate]]
